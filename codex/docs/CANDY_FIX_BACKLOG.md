...
| HP-ASSET-DELETE | Unconfirmed-reference, duplicate-hash, and possibly unused public assets | `generated/CANDY_CODE_ASSET_INVENTORY.md` detects candidates, but machine analysis cannot rule out dynamic references | Verify PHP, database, JavaScript, production references, and recovery methods, then obtain the target decision through the cumulative deletion route | `AWAITING_APPROVAL` |
...
| INDEX-PRODUCTION | `HP/index.php` | This protected target affects final production switchover | Resolve the final-switchover target through the cumulative production route with target SHA, redirects, rollback method, and HTTP checks | `AWAITING_APPROVAL` |
