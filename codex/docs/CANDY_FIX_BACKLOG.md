...
| RENEWAL-ENTRY-CONTRACT | Production entry management, verification, and deployment workflows | The local canonical documents and verification code define and pass root `200`, explicit-index removal, canonical scheme/host redirects, public indexability, and direct-host noindex; Commit and Push are authorized, while the updated Actions run is pending | Commit and Push only the renewal-entry correction, verify the updated Actions workflow and live entry contract, then remove this completed row before resuming the preserved `nishisakamotocho` publication state | `IN_PROGRESS` |
