...
|---|---:|---|
| `01_間違い無し` | 97 | No classification issue; page bundle is not yet complete |
...
| `09_作成済み` | 41 | Public PHP, source HTML, dataset PHP, and the required `dataset_base.php` registrations are complete |
| **Total** | **156** | Inventoried Text inputs |
...
|---|---:|---|---|
| Necessary retained Text | 158 | 156 inventoried inputs plus two retained inputs outside the TSV inventory | `Text_area_data/分類_20260716_115215/` |
| Necessary source images | 354 | All files are valid 1000×750 JPEG data; 177 complete `_1`/`_2` filename pairs are retained | `Text_area_data/画像データ/` |
| Necessary classification inventory | 1 | The 156 TSV rows match the 156 inventoried Text files one-to-one; two retained inputs remain outside the inventory | `Text_area_data/分類_20260716_115215/分類結果.tsv` |
...
All 156 inventoried files were read as UTF-8 and passed one-to-one inventory checks.

All 354 retained JPEG files are readable. `kagoshima-deliveryhealth-area-ishikidai_1.jpg` and `_2.jpg` have identical content, but both remain because they are the two explicitly named correction targets for the classified `ishikidai` input; they are not treated as an extra backup copy.
