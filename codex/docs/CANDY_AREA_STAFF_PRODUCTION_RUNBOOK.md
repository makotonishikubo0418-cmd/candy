...
> 配置変更後のスクリプト内部パス移行が未完了のため、修正とdry-run検証が終わるまでコマンドを実行しない。
...
1. `Text_area_data` 直下にあるtxtをファイル名昇順で確認する。
2. 最新の `Text_area_data/分類_*/01_間違い無し` をファイル名昇順で確認する。
