...
Codex管理正本は `\\192.168.1.3\disk1\FSG_SEO\candy\codex` に置く。プロジェクト管理文書は `codex/project_management/`、HP制作仕様は `codex/docs/`、作業ツールは `codex/scripts/` を使用する。

共有ルートには共通入口 `AGENTS.md` とGit管理に必要なファイルだけを置き、同じ管理文書を複製しない。`HP/` は実サイト配下専用とし、管理正本を置かない。
...
- 仕様と履歴が混ざっていない
- 未確認を完了扱いしていない
- 管理文書の正本が `codex/` にあり、共有ルートやHP側に重複していない

## 7. Git Commit・Push高速実行ルール

- 開始時に今回の対象ファイルを固定する。対象外の変更、削除、未追跡は確認、Stage、Commitへ含めない。
- `AGENTS.md`、`codex/README.md`、Branch、Remote、対象差分は開始時に1回だけ確認する。
...
```powershell
$managementRoot = '\\192.168.1.3\disk1\FSG_SEO\candy\codex'
...
Test-Path -LiteralPath (Join-Path $managementRoot 'project_management\DOCUMENT_RULES.md')
Test-Path -LiteralPath (Join-Path $repo 'HP\AGENTS.md')
Test-Path -LiteralPath (Join-Path $repo 'HP\index.php')
