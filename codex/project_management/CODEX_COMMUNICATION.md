...
| COMM-20260716-005 | 2026-07-16 | 全Codex | Git操作は `C:\Codex\candy` だけで行う。開始時に `git fetch origin` と `git status --short --branch` を確認し、behindなら先にpullする。Pushは明示指示時だけ行い、NASではGit操作しない | Git同期 | 有効 |
...
| COMM-20260716-012 | 2026-07-16 | 全Codex | Git作業場は `C:\Codex\candy`。GitHubを同期ハブとし、NAS `\\192.168.1.3\disk1\FSG_SEO\candy` は `Backup/`・受入素材の保管専用とする。`HP/HP/` と `HP/README.md` は作らない | 作業場所・HP階層 | 有効 |
...
| COMM-20260717-014 | 2026-07-17 | 全Codex | 管理入口は `codex/README.md`、プロジェクト管理正本は `codex/project_management/`、HP制作仕様は `codex/docs/`。ローカルリポジトリ直下、HP配下、NASへ管理正本を複製しない | 管理文書 | 有効 |
