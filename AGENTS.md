...
2026-07-13 に `C:\Users\nishi\Desktop\data\candy` の実フォルダと Git 状態から確認した構成です。
...
### 12.1 「全て」「100％」「漏れなく」の絶対定義

- ユーザーが「全て」「100％」「漏れなく」と指定した場合、抜粋確認、代表確認、推測、過去結果の流用で完了としてはいけない
- 指定範囲の全対象を列挙または機械集計し、確認総数、確認方法、例外、未確認数を記録する
- 未確認が1件でもある場合は「100％確認済み」「全て確認済み」と報告しない
- フォルダが再取得、差替え、生成、更新された場合、以前の件数・ハッシュ・結論を無効として実ファイルから再確認する
- チャット貼付内容、過去資料、以前の調査結果と現在の実ファイルが異なる場合、現在の実ファイルを優先して差異を明示する
- 実ファイルを読まずに、ファイル名、過去の報告、ユーザー説明だけから内容を断定しない
- 「最高品質」の指定は、対象全体の再確認、矛盾検索、差分検証、未確認事項の明示までを含む
...
- サーバー環境は、`/public_html/group/candy/`が本番、`/public_html/group_test/candy/`が制作時のテスト版である。この区分を混同しない
- 最新HP内の`group_test/candy`絶対参照はテスト版向けであり、本番移行の最終切替対象とする。ユーザー承認なしに一括置換しない
- HPの段階移行・本番反映前に、`HP/codex/docs/CANDY_PRODUCTION_MIGRATION_MASTER.md`と`CANDY_PRODUCTION_MIGRATION_INVENTORY.csv`を必ず確認する
- 移行マスターのGate 0が完了するまで、自動反映対象のHPファイルを通常運用としてPushしない。例外は、対象と本番影響を限定してユーザーが明示承認した検証だけとする
...
### 13.1 段階移行中のトップ転送と最終公開切替

- 2026-07-13に実サーバーから再取得した`HP_旧データ`を、取得時点の本番スナップショットとして扱う
- `HP_旧データ/index.php`は、サイトトップをシティヘブンのCANDYページへ301転送する入口ファイルである
- `HP_旧データ/.htaccess`は`DirectoryIndex index.php index.html index.xhtml`で、`index.php`を最優先する
- 段階移行中は本番の転送用`index.php`を絶対に上書きしない。その他の承認済みファイルを先行反映しても転送を維持する
- 最新`HP/index.php`の本番配置は「最終公開切替」である。ユーザーが公開切替を明示承認した場合だけ最後に上書きする
- 最新`HP/index.php`を上書きした時点で、シティヘブン転送を終了し、新しいCANDYサイトを公開する
- 段階移行中は、workflowのpaths除外とデプロイスクリプトの保護対象によって`HP/index.php`を二重に除外する。この二重保護を解除する変更は最終公開切替として扱う
- 公開切替前後に、301転送、トップ表示、主要ページ、PC/SP、HTTP状態を実確認する
- 初回移行ではユーザーの明示承認により、転送用`index.php`を除くGit管理中の本番用ファイルを一度だけ全件反映できる
- 全件反映でも`HP/index.php`、管理資料、元データ、ログ、`.well-known`は対象外とし、サーバーファイルを削除しない
- 全件反映後もトップはシティヘブン転送を維持する。その他のPHPは直接URLで新内容が表示され得るため、確認用URLとして扱う
