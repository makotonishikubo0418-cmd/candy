...
このファイルは、Codex が candy プロジェクトを扱うときの最上位ルールです。
...
| 現在の作業ルート | `\\192.168.1.3\disk1\FSG_SEO\candy\HP` |
| Codex 管理フォルダ | `\\192.168.1.3\disk1\FSG_SEO\candy\HP\codex` |
| Codex 既存資料 | `\\192.168.1.3\disk1\FSG_SEO\candy\HP\codex\docs` |
| サイト種別 | PHP テンプレート生成型サイト |
| 主な用途 | 店舗案内、女の子一覧、出勤情報、料金案内、動画、対応エリア、ホテル情報、問い合わせ導線 |

既存資料内には `H:\Data\01_CTI\candy_HP` というパス表記があります。  
この表記は過去または別環境のパスとして扱い、現在の作業では上記ネットワークパスを確認済みルートとして扱ってください。

`H:\Data\01_CTI\candy_HP` と現在のネットワークパスが完全に同一かは、必要時に確認してください。未確認のまま同一と断定してはいけません。
...
| 2 | `HP\codex\docs\CANDY_MASTER_DOC_INDEX.md` | 現行管理資料の入口 |
| 3 | `HP\codex\docs\CANDY_FULL_FILE_CODE_INVENTORY.md` | 全フォルダ/全ファイル台帳 |
| 4 | `HP\codex\docs\CANDY_CODE_FILE_STRUCTURE.md` | PHP/HTML/dataset/CSS/JS/設定構成 |
| 5 | `HP\codex\docs\CANDY_NON_CODE_ASSET_INVENTORY.md` | 非コード資産、ログ、DB、TXT/CSV |
| 6 | `HP\codex\docs\CANDY_FOLDER_ROLE_MAP.md` | 全フォルダ役割 |
| 7 | `HP\codex\docs\CANDY_PAGE_SPEC_INDEX.md` | ページ別役割 |
| 8 | `HP\codex\docs\CANDY_PAGE_CATEGORY_STRUCTURE.md` | カテゴリ別ページ構成 |
| 9 | `HP\codex\docs\CANDY_CODEX_BACKUP_REMARKS.md` | codex_backup備考、旧資料削除後の扱い |
| 10 | `HP\codex\docs\CANDY_EXISTING_DOCS_INVENTORY.md` | 旧資料削除後の棚卸記録 |
| 11 | `HP\codex\docs\CANDY_PHASE_RECHECK.md` | フェーズ別再確認 |
...
- 改修は指示された対象ファイルのみ変更する。1回の指示で触るのは最大10ファイルまで。
- 変更前に git status がcleanであることを確認する。
- 変更後は git diff --stat と変更内容の要約を報告し、承認を得るまで次の変更に着手しない。
