...
// クッキー用ドメイン設定
$club_domains = array();
$club_domains[1] = "firststar.kir.jp";
$club_domains[2] = "firststar.kir.jp";
$club_domains[3] = "firststar.kir.jp";
$club_domains[4] = "firststar.kir.jp";
...
// デバッグログをファイルに書き込み（お気に入り解除処理後に移動）
...
if ($ROWS != 0) {
	while ($row = $Database->Fetch_Array($RESULT)) {
		...
		/*if($row["horw"] != ""){
			$imagedata["filename"][$row["girls_id"]][$row["horw"]][] = $row["filename"];
		}/*else{
			$imagedata["filename"][$row["girls_id"]][0][] = $row["filename"];
		}*/
	}
}
...
if ($unf != "") {
	...
	if ($girl_id != "" && count($temp_favcast) > 0) {
		// 配列から削除
		$key = array_search($girl_id, $temp_favcast);
		if ($key !== false) {
			unset($temp_favcast[$key]);
			$temp_favcast = array_values($temp_favcast); // インデックスを再構築
			
			// クッキーを更新
			$cookie_value = urlencode(implode(',', $temp_favcast));
...
while ($i < count($favcast)) {
	...
	// レスポンシブ対応のため、画像サイズを統一
	if ($imagedata["filename"][$gid][0] != "") {
		$imgsize = 310;
		$imgfile = $imagedata["filename"][$gid][0];
		$imgtype = 'w';
	} else {
		$imgsize = 310;
	...
	//
	$waku1 = str_replace('rep03010091eot', 'mypage.php?unf=' . $girldata["no"][$gid], $waku1); //URI
	...
	if ($sche_type == 1) { //出勤(受付中)
		...
		//
		$waku1 = str_replace('mypage.php?unf=', 'mypage.php?kog=g&unf=', $waku1); //
...
if (isset($_COOKIE["candyfav"])) {
	if (count($favcast) > 0) {
		...
		//インフォメーションテキスト取得
		$infodata = array();
		$QUERY  = "SELECT no,detail FROM shop_mypage_info";
		$QUERY .= " WHERE club_id = '" . CLUBID . "'";
		$QUERY .= " AND no = '1'";
		$QUERY .= " AND status = 1";
		$QUERY .= " ORDER BY no LIMIT 1";
		$RESULT = $Database->Query($QUERY);
		$ROWS = $Database->Num_Rows($RESULT);
		if ($ROWS != 0) {
			while ($row = $Database->Fetch_Array($RESULT)) {
				$infodata["no"]          = $row["no"];
				$infodata["detail"]    = $row["detail"];
			}
		}
		$infodata["detail"] = str_replace('&lt;', '<', $infodata["detail"]);
		$infodata["detail"] = str_replace('&gt;', '>', $infodata["detail"]);
		$infodata["detail"] = str_replace('&quot;', '"', $infodata["detail"]);
		$infodata["detail"] = str_replace('&amp;', '&', $infodata["detail"]);
		$data1['00010402'] = nl2br($infodata["detail"]);

		//コメントイン
		$source = str_replace('<!-- MYPAGEINFO', '<!-- MYPAGEINFO -->', $source);
		$source = str_replace('/MYPAGEINFO -->', '<!-- /MYPAGEINFO -->', $source);
	}
}
