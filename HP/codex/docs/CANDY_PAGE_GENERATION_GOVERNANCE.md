...
制作完了後に「アップしろ」と指示された場合は、今回の制作記録を関連 `.md` へ反映し、対象だけをCommit・Pushして自動本番Actionsを追跡し、本番URLのHTTP確認まで連続実行します。正常時は途中確認を返しません。Actions確認はGitHub APIを使い、ブラウザUI操作を通常経路にしません。
