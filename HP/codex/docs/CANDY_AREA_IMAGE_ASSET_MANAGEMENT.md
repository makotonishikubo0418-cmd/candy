...
| 区分 | パス | 扱い |
|---|---|---|
| 準備・受入用 | `画像データ/area_img` | ユーザーが準備した画像を一時確認する場所 |
| 公開用正本 | `HP/imgHtml/new_202601/area` | HTMLから参照する実画像の配置先 |
| HTML参照 | `./imgHtml/new_202601/area/<ファイル名>` | area source HTML内の参照形式 |

`画像データ/area_img`を公開用正本として参照してはいけません。公開ページは `HP/imgHtml/new_202601/area` を参照します。
...
```text
C:\Users\nishi\Desktop\data\candy\画像データ\area_img
```
...
新規areaページの依頼時に `_1`・`_2` の必要画像がない場合は、制作を停止してユーザーへ報告し、正式ファイル名を示して提供を依頼します。既存画像の無断流用、ダミー画像、画像名の推測、画像なしでの公開は禁止します。
...
- `画像データ/area_img` は準備・受入用であり、同一画像を二重Commitしない
- 現在 `画像データ` は未追跡である
- `.gitignore` は現在存在しない
