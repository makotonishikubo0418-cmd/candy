...
- 完成した通常sceneは入力順で保持し、不完全なsceneは生成前に停止する
...
6. `template_shop.html`から指定店舗ブロックを反映し、Text未指定時だけ地図座標と近隣完成areaページから移動時間・交通費を設定する
7. 完成した通常sceneを入力順に反映し、FAQを元データの件数に合わせて構成する
